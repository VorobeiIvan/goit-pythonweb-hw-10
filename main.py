...
def get_current_user(
    ...
    try:
        ...
        redis_client.setex(
            f"user:{email}", timedelta(minutes=30), json.dumps(user_dict)
        )
...
def update_avatar(
    ...
    if current_user.role != UserRole.ADMIN:
        raise HTTPException(
            status_code=403, detail="Only admin users can change their avatar"
        )
    ...
    try:
        ...
        redis_client.setex(
            f"user:{current_user.email}", timedelta(minutes=30), json.dumps(user_dict)
        )
...
def login(
    ...
    redis_client.setex(
        f"user:{user.email}", timedelta(minutes=30), json.dumps(user_dict)
    )
...
def send_password_reset_email(email: str, token: str):
    ...
    msg["To"] = email
    msg.set_content(
        f"Click the link to reset your password: http://127.0.0.1:8000/reset-password/{token}"
    )
...
def forgot_password(email: EmailStr, db: Session = Depends(get_db)):
    ...
    reset_token = create_access_token(
        {"sub": email, "type": "password_reset"}, expires_delta=timedelta(minutes=15)
    )
